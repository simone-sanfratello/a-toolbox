...
},{"./hash/common":2,"./hash/hmac":3,"./hash/ripemd":4,"./hash/sha":5,"./hash/utils":12}],2:[function(require,module,exports){
'use strict';
...
},{"./common":2,"./utils":12}],5:[function(require,module,exports){
'use strict';
...
},{"./sha/1":6,"./sha/224":7,"./sha/256":8,"./sha/384":9,"./sha/512":10}],6:[function(require,module,exports){
'use strict';
...
},{"../common":2,"../utils":12,"./common":11}],7:[function(require,module,exports){
'use strict';
...
},{"../utils":12,"./256":8}],8:[function(require,module,exports){
'use strict';
...
},{"../utils":12,"./512":10}],10:[function(require,module,exports){
'use strict';
...
'use strict';window.tools={array:require('./array'),hash:require('./hash'),object:require('./object'),event:require('./event'),random:require('./random'),string:require('./string'),task:require('./task'),time:require('./time'),util:require('./util')};
...
"use strict";var event={Emitter:function Emitter(){var __listners={};var __onceListeners={};var emit=function emit(name){if(__listners[name]){for(var _len=arguments.length,values=Array(_len>1?_len-1:0),_key=1;_key<_len;_key++){values[_key-1]=arguments[_key]}var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=__listners[name][Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var _listener=_step.value;_listener.apply(null,values)}}catch(err){_didIteratorError=true;_iteratorError=err}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return()}}finally{if(_didIteratorError){throw _iteratorError}}}if(__onceListeners[name]){var _iteratorNormalCompletion2=true;var _didIteratorError2=false;var _iteratorError2=undefined;try{for(var _iterator2=__onceListeners[name][Symbol.iterator](),_step2;!(_iteratorNormalCompletion2=(_step2=_iterator2.next()).done);_iteratorNormalCompletion2=true){var _listener2=_step2.value;_listener2.apply(null,values)}}catch(err){_didIteratorError2=true;_iteratorError2=err}finally{try{if(!_iteratorNormalCompletion2&&_iterator2.return){_iterator2.return()}}finally{if(_didIteratorError2){throw _iteratorError2}}}delete __onceListeners[name]}}};var on=function on(name,callback){if(!__listners[name]){__listners[name]=[]}__listners[name].push(callback)};var once=function once(name,callback){if(!__onceListeners[name]){__onceListeners[name]=[]}__onceListeners[name].push(callback)};var off=function off(name){delete __listners[name];delete __onceListeners[name]};return{emit:emit,on:on,off:off,once:once}}};module.exports=event;

},{}],18:[function(require,module,exports){
'use strict';var lib=require('hash.js');var hash={sha256:function sha256(data){return lib.sha256().update(data).digest('hex')}};module.exports=hash;
...
'use strict';var tools={object:require('./object')};var string={template:function template(str,obj){var remove=arguments.length>2&&arguments[2]!==undefined?arguments[2]:false;if(!str){return''}var _obj=tools.object.flat(obj);return str.replace(/\{([\w.]+)\}/g,function(str,key){return _obj[key]?_obj[key]:remove?'':str})},trim:function trim(str,cuts){if(!cuts){return str.replace(/^[\s\xa0]+|[\s\xa0]+$/g,'')}else{var _words=void 0;var _cuts=cuts.map(function(c){if(c.length>1){_words=true;return c}if(c==='['||c===']'){return'\\'+c}return c});if(_words){var _subexpression2=_cuts.join('|');return str.replace(new RegExp('^('+_subexpression2+')+|('+_subexpression2+')+$','gm'),'')}var _subexpression=_cuts.join();return str.replace(new RegExp('^['+_subexpression+']+|['+_subexpression+']+$','gm'),'')}},replaceAll:function replaceAll(str,from,to){if(!str){return''}return str.split(from).join(to)},capitalize:function capitalize(str){return str.substr(0,1).toUpperCase()+str.substr(1).toLowerCase()},prependMissing:function prependMissing(prefix,str){if(str.indexOf(prefix)===0){return str}return prefix+str},matchAll:function matchAll(str,regexp){var _matches=[];var _match=regexp.exec(str);while(_match){_matches.push(_match);_match=regexp.exec(str)}return _matches}};module.exports=string;

},{"./object":19}],22:[function(require,module,exports){
'use strict';var time=require('./time');var array=require('./array');var task={Worker:function Worker(){var options=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};var __tasks=[];this.todo=function(id){if(options.chrono){time.chrono.set(id)}__tasks.push(id)};this.done=function(id){array.remove(__tasks,id);if(__tasks.length<1){if(options.done){options.done()}}if(options.chrono){var _time=time.chrono.get(id);time.chrono.clear(id);return{chrono:_time}}}}};module.exports=task;

},{"./array":16,"./time":23}],23:[function(require,module,exports){
'use strict';var object=require('./object');var time={chrono:{set:function set(){var tag=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'chrono';if(_chronos[tag]){console.warn('time chrono',tag,'already set; use reset instead or start a new chrono');return}_chronos[tag]=Date.now()},reset:function reset(){var tag=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'chrono';_chronos[tag]=Date.now()},clear:function clear(){var tag=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'chrono';delete _chronos[tag]},get:function get(){var tag=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'chrono';var _now=Date.now();return _chronos[tag]?_now-_chronos[tag]:0}},gc:function gc(){object.empty(_chronos)}};var _chronos={};module.exports=time;

},{"./object":19}],24:[function(require,module,exports){
'use strict';var _typeof=typeof Symbol==='function'&&typeof Symbol.iterator==='symbol'?function(obj){return typeof obj}:function(obj){return obj&&typeof Symbol==='function'&&obj.constructor===Symbol&&obj!==Symbol.prototype?'symbol':typeof obj};var util={isSet:function isSet(val){return typeof val!=='undefined'&&val!==null},onBrowser:function onBrowser(){return(typeof window==='undefined'?'undefined':_typeof(window))==='object'&&window instanceof Window}};module.exports=util;
